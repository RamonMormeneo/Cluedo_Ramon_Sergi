...
{
	sf::RenderWindow _window(sf::VideoMode(800, 800), "Ventanita");
...
void Graphics::DrawDungeon(PlayerInfo Players[6])
{
	sf::RenderWindow _window(sf::VideoMode(800, 800), "Ventanita");
	sf::RectangleShape shape(sf::Vector2f(SIZE, SIZE));
	shape.setOutlineColor(sf::Color::Black);
	...
	while (_window.isOpen())
	{
		...
		centroMensajes.Draw(_window);
		//Dibujo PJ
		sf::Vector2f position = (sf::Vector2f)Players[1].position;
		//position.x = 0; position.y = 0;
		shape.setFillColor(sf::Color::Blue);
		shape.setFillColor(sf::Color(0, 0, 255, 255));
		shape.setPosition(sf::Vector2f(position.x*SIZE, position.y*SIZE));
